  ...
  return (
    <main className="min-h-screen bg-[#fffaf4] text-[#4a281f]">
      ...
      <section className="moony-paper border-b border-[#5b2f22]/10">
        <div className="mx-auto grid max-w-[1500px] gap-10 px-6 py-16 lg:grid-cols-[.78fr_1.22fr] lg:px-12 lg:py-24">
          <div className="max-w-[500px]">
            ...
            <h2 className="moony-serif mt-4 text-[48px] leading-[.98] tracking-[-.04em] sm:text-[58px]">
              Pensée pour la vraie vie des femmes.
            </h2>
            <p className="mt-6 text-[16px] leading-7 text-[#5b2f22]/72">
              MOONY rassemble dans une même expérience des repères de santé, des ressources, une communauté et l’accès à des professionnelles. L’objectif : rendre le parcours plus lisible, plus humain et plus accessible.
            </p>
            <Link href="/notre-approche" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-[#8d3b19]">
              Découvrir notre approche <ArrowUpRight size={16} />
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-[1500px] px-6 py-16 lg:px-12 lg:py-20">
          <div className="grid gap-10 lg:grid-cols-[1fr_1.15fr] lg:items-end">
            <div>
              ...
              <h2 className="moony-serif mt-4 max-w-[630px] text-[46px] leading-[1] tracking-[-.04em] sm:text-[58px]">
                La confiance n’est pas une option. C’est une base.
              </h2>
            </div>
            <p className="max-w-[650px] text-[16px] leading-7 text-white/68 lg:justify-self-end">
              Une plateforme de santé doit expliquer clairement ce qu’elle fait, pourquoi elle collecte certaines informations et comment elles sont protégées. MOONY construit son expérience autour de la confidentialité, de la transparence et de la minimisation des données.
            </p>
          </div>
          ...
          <Link href="/confidentialite" className="mt-8 inline-flex items-center gap-2 rounded-full border border-white/25 px-6 py-3 text-sm font-medium hover:bg-white/7">
            En savoir plus sur notre engagement <ArrowUpRight size={15} />
          </Link>
        </div>
      </section>
      ...
      <section className="border-b border-[#5b2f22]/10 bg-[#fffaf4]">
        <div className="mx-auto max-w-[1500px] px-6 py-16 lg:px-12 lg:py-24">
          <div className="max-w-[800px]">
            <p className="text-[11px] font-semibold uppercase tracking-[.3em] text-[#9d4c27]">Pour les professionnelles & les organisations</p>
            <h2 className="moony-serif mt-4 text-[48px] leading-[.98] tracking-[-.04em] sm:text-[58px]">MOONY relie aussi celles et ceux qui accompagnent les femmes.</h2>
          </div>
          ...
          <div className="mt-9 grid gap-4 lg:grid-cols-2">
            <article className="rounded-[28px] border border-[#5b2f22]/10 bg-[#f7e8dd] p-7 sm:p-9">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3"><Link href="/services#professionnels" className="rounded-full bg-[#7e3518] px-6 py-3 text-sm font-medium text-white">Voir les offres</Link><span className="text-xs text-[#5b2f22]/55">À partir de 15 000 FCFA / mois</span></div>
            </article>
          ...
          <div className="rounded-[32px] border border-[#5b2f22]/10 bg-[#ba693f] p-8 text-[#fff8f1] shadow-[0_20px_60px_rgba(93,49,32,.1)] sm:p-10">
            <div className="mx-auto flex min-h-[300px] max-w-[460px] items-center justify-center rounded-[26px] border border-white/15 bg-[radial-gradient(circle_at_50%_20%,rgba(255,224,194,.28),transparent_38%),linear-gradient(145deg,#a65331,#c98058)]">
              <div className="text-center">
                <div className="mx-auto grid h-24 w-24 place-items-center rounded-full border border-white/25 bg-white/10"><Leaf size={42} strokeWidth={1.1} /></div>
                <p className="moony-serif mt-5 text-3xl">MOONY Réutilisable</p>
              </div>
            </div>
          </div>
          <div>
      ...
      <section className="bg-[#fffaf4]">
        <div className="mx-auto max-w-[1500px] px-6 py-16 lg:px-12 lg:py-24">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div><p className="text-[11px] font-semibold uppercase tracking-[.3em] text-[#9d4c27]">Ressources</p><h2 className="moony-serif mt-3 text-[46px] leading-none sm:text-[56px]">Comprendre, apprendre, avancer.</h2></div>
            <Link href="/ressources" className="inline-flex items-center gap-2 text-sm font-semibold text-[#8d3b19]">Voir toutes les ressources <ArrowUpRight size={15} /></Link>
          </div>
          <div className="mt-9 grid gap-4 lg:grid-cols-3">
            {resourceCards.map((card, index) => (
              <article key={card.title} className="group overflow-hidden rounded-[24px] border border-[#5b2f22]/10 bg-white">
                <div className={`h-44 ${index === 0 ? "bg-[linear-gradient(135deg,#6e3b2b,#c1815e)]" : index === 1 ? "bg-[linear-gradient(135deg,#ddbea7,#a86845)]" : "bg-[linear-gradient(135deg,#89523d,#e3b69a)]"}`} />
                <div className="p-6"><span className="text-[10px] font-semibold tracking-[.18em] text-[#a15837]">{card.tag}</span><h3 className="moony-serif mt-3 text-[29px] leading-[1.04]">{card.title}</h3><p className="mt-3 text-sm leading-6 text-[#5b2f22]/64">{card.body}</p><Link href="/ressources" className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-[#8d3b19]">Lire <ArrowUpRight size={14} /></Link></div>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-[#5b2f22]/10 bg-[#f5e6da]">
        <div className="mx-auto grid max-w-[1500px] gap-8 px-6 py-14 lg:grid-cols-[1fr_auto] lg:items-center lg:px-12">
          <div><p className="text-[11px] font-semibold uppercase tracking-[.28em] text-[#9d4c27]">L’expérience MOONY</p><h2 className="moony-serif mt-3 text-[42px] leading-[1.02] sm:text-[50px]">Votre santé. Votre rythme. Votre espace.</h2><p className="mt-3 text-sm leading-6 text-[#5b2f22]/65">Accédez à MOONY et retrouvez vos parcours, vos ressources et votre communauté.</p></div>
          <a href={siteConfig.appUrl} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#7e3518] px-7 py-3.5 text-sm font-medium text-white">Accéder à l’application <ArrowUpRight size={15} /></a>
        </div>
      </section>
