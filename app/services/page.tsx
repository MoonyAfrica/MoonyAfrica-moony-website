...
const fallbackHero = {
  ...
  title: "Des solutions pensées pour chaque étape de la vie",
  body: "Prévention, accompagnement, soins et bien-être : MOONY réunit des services utiles, accessibles et fiables pour les femmes, les professionnels de santé et les organisations.",
};

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPublishedPage("/services");
  return {
    title: page?.seo_title || "Nos services — MOONY Africa",
    description: page?.seo_description || page?.hero?.body || fallbackHero.body,
  };
}

export default async function ServicesPage() {
  const [page, pricing] = await Promise.all([getPublishedPage("/services"), getPublicPricing()]);
  const hero = page?.hero ?? {};
  const activePlans = pricing.plans.filter((plan) => plan.active !== false);
    ...
    <main className="min-h-screen bg-[#fbf5ee] text-[#5b2f22]">
      <section className="relative overflow-hidden border-b border-[#5b2f22]/10 bg-[linear-gradient(100deg,#fbf5ee_0%,#f2dfcc_64%,#c9865b_100%)] pt-36">
        ...
        <div className="mx-auto max-w-[1500px] px-6 pb-12 lg:px-12">
          <p className="text-[11px] font-semibold uppercase tracking-[.35em] text-[#a45b35]">{hero.eyebrow || fallbackHero.eyebrow}</p>
          <h1 className="moony-serif mt-3 max-w-[900px] whitespace-pre-line text-5xl leading-[.98] tracking-[-.045em] sm:text-6xl lg:text-7xl">{hero.title || fallbackHero.title}</h1>
          <p className="mt-5 max-w-[780px] text-[17px] leading-7 text-[#5b2f22]/75">{hero.body || fallbackHero.body}</p>
        </div>
      </section>
      ...
      <section id="professionnels" className="mx-auto max-w-[1500px] scroll-mt-24 px-6 py-14 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <p className="rounded-full bg-[#f1dfd2] px-4 py-2 text-xs text-[#6d3a2b]">Téléconsultations : commission MOONY de {pricing.teleconsultationCommission}%</p>
        </div>

        <div className={`mt-7 grid gap-5 ${activePlans.length >= 4 ? "xl:grid-cols-4" : activePlans.length === 3 ? "xl:grid-cols-3" : "xl:grid-cols-2"}`}>
          {activePlans.map((plan) => (
            <article key={plan.key} className={`relative rounded-[22px] border bg-white/72 p-7 ${plan.featured ? "border-[#d97b50] shadow-[0_18px_45px_rgba(112,50,27,.09)]" : "border-[#5b2f22]/12"}`}>
              ...
              <p className="moony-serif mt-3 text-[30px]">{formatPrice(plan.price)} {pricing.currency} <span className="font-sans text-sm text-[#5b2f22]/60">{plan.unit}</span></p>
              <ul className="mt-5 space-y-2 text-sm text-[#5b2f22]/75">
                {benefits.map((benefit) => <li key={benefit} className="flex gap-2"><Check className="mt-[2px] shrink-0 text-[#a45332]" size={15} /> <span>{benefit === "Téléconsultation" ? `${benefit} — commission ${pricing.teleconsultationCommission}%` : benefit}</span></li>)}
              </ul>
