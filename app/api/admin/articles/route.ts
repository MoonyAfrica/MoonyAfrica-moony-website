...
export async function POST(request: Request) {
  ...
  const { data, error: insertError } = await supabase.from("website_articles").insert({
    ...
    seo_title: asNullableText(input.seoTitle, 240), seo_description: asNullableText(input.seoDescription, 360), author_name: asText(input.authorName, 160) || "MOONY Africa", updated_at: now,
  }).select("*").single();
...
export async function PATCH(request: Request) {
  ...
  if ("authorName" in input) patch.author_name = asText(input.authorName, 160) || "MOONY Africa";
