...
export async function GET(request:Request){
 ...
 for(const row of invoices.data??[]){if(!["issued","overdue"].includes(row.status))continue;const values=bucket(row.currency||"XOF"),amount=Number(row.total_amount||0);values.outstanding+=amount;if(row.status==="overdue")values.overdue+=amount;if(row.due_date&&row.due_date>=now&&row.due_date<=in30)values.due30+=amount}
