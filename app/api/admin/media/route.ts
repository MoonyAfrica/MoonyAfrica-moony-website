...
export async function POST(request:Request){
 ...
 if(insertError){await supabase.storage.from(BUCKET).remove([path]);return NextResponse.json({error:insertError.message},{status:500})}
 await writeAuditLog(supabase,session,"content.media_uploaded","media",data.id,`Média « ${file.name} » importé`,{folder,mimeType:file.type,fileSize:file.size});
 return NextResponse.json({media:data},{status:201});
}

export async function PATCH(request:Request){
 const {error,supabase,session}=requireAdmin(request,"content.write");if(error||!supabase)return error;let body:Record<string,unknown>;try{body=await request.json()}catch{return NextResponse.json({error:"Requête invalide."},{status:400})}
 ...
 const {data,error:queryError}=await supabase.from("website_media").update(patch).eq("id",id).select().single();if(queryError)return NextResponse.json({error:queryError.message},{status:500});
 await writeAuditLog(supabase,session,"content.media_updated","media",id,`Média « ${data.file_name} » modifié`,{folder:data.folder,altText:data.alt_text});
 return NextResponse.json({media:data});
}

export async function DELETE(request:Request){
 const {error,supabase,session}=requireAdmin(request,"content.write");if(error||!supabase)return error;const id=text(new URL(request.url).searchParams.get("id"),80);if(!id)return NextResponse.json({error:"ID manquant."},{status:422});
 const {data,error:readError}=await supabase.from("website_media").select("storage_path,file_name,folder,mime_type").eq("id",id).maybeSingle();if(readError)return NextResponse.json({error:readError.message},{status:500});if(!data)return NextResponse.json({error:"Média introuvable."},{status:404});
 const {error:removeError}=await supabase.storage.from(BUCKET).remove([data.storage_path]);if(removeError)return NextResponse.json({error:removeError.message},{status:500});const {error:deleteError}=await supabase.from("website_media").delete().eq("id",id);if(deleteError)return NextResponse.json({error:deleteError.message},{status:500});
 await writeAuditLog(supabase,session,"content.media_deleted","media",id,`Média « ${data.file_name} » supprimé`,{folder:data.folder,mimeType:data.mime_type});
