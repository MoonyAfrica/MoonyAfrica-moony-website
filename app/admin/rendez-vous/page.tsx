...
import { CalendarDays, ExternalLink, Loader2, Plus, Trash2, Video } from "lucide-react";
...
export default function AppointmentsPage() {
  ...
  async function createAppointment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await load();
      setCreating(false);
      setNotice("Rendez-vous planifié et rattaché au CRM.");
    } catch (error) {
  ...
  async function updateStatus(status: AppointmentStatus) {
    ...
    try {
      ...
      setNotice("Statut du rendez-vous mis à jour.");
    } catch (error) {
  ...
  return (
    <AdminWorkspace
      ...
      actions={<button onClick={() => setCreating(true)} className="inline-flex items-center gap-2 rounded-lg bg-[#7e3518] px-5 py-2.5 text-sm text-white"><Plus size={15} /> Planifier un rendez-vous</button>}
    >
