...
export function AdminMediaPicker({ value = "", label = "Choisir dans la bibliothèque", onSelect }: Props) {
  ...
  async function load(search = query) {
    ...
    try {
      ...
      setAssets((data.media ?? []).filter((item: MediaAsset) => item.mime_type?.startsWith("image/")));
    } catch (reason) {
          ...
          <div className="flex max-h-[86vh] w-full max-w-5xl flex-col overflow-hidden rounded-[22px] border border-white/20 bg-[#fbf8f4] shadow-2xl">
            <div className="flex items-center gap-3 border-b border-[#5b2f22]/10 bg-white px-5 py-4">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-[#5b2f22]/48">Sélectionnez une image déjà importée dans le Control Center.</p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-[#5b2f22]/8 px-5 py-3">
              <div className="flex min-w-[240px] flex-1 items-center gap-2 rounded-lg border border-[#5b2f22]/10 bg-white px-3">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void load(); }} placeholder="Rechercher une image…" className="w-full bg-transparent py-2.5 text-xs outline-none" />
              </div>
              ...
              <a href="/admin/medias" target="_blank" className="rounded-lg bg-[#7e3518] px-4 py-2.5 text-xs font-medium text-white">Importer une image ↗</a>
            </div>

            <div className="min-h-0 flex-1 overflow-y-auto p-5">
              {loading ? <div className="grid min-h-[280px] place-items-center text-sm text-[#5b2f22]/48"><span className="inline-flex items-center gap-2"><Loader2 className="animate-spin" size={17} /> Chargement des images…</span></div> : null}
              {!loading && error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div> : null}
              {!loading && !error && !assets.length ? <div className="grid min-h-[280px] place-items-center text-center"><div><ImageIcon className="mx-auto text-[#5b2f22]/22" size={38} /><p className="mt-3 text-sm text-[#5b2f22]/48">Aucune image trouvée.</p><a href="/admin/medias" target="_blank" className="mt-3 inline-block text-xs font-semibold text-[#8d3b19]">Ouvrir la bibliothèque pour importer →</a></div></div> : null}
              {!loading && assets.length ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{assets.map((asset) => (
                <button key={asset.id} type="button" onClick={() => { onSelect({ url: asset.public_url, altText: asset.alt_text ?? "", fileName: asset.file_name }); setOpen(false); }} className={`group overflow-hidden rounded-xl border bg-white text-left transition hover:-translate-y-0.5 hover:shadow-md ${selectedId === asset.id ? "border-[#b65e39] ring-2 ring-[#b65e39]/20" : "border-[#5b2f22]/9 hover:border-[#b65e39]/35"}`}>
                  <div className="relative h-40 bg-[#efe4dc]"><img src={asset.public_url} alt={asset.alt_text ?? ""} className="h-full w-full object-cover" />{selectedId === asset.id ? <span className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-full bg-[#7e3518] text-white"><Check size={14} /></span> : null}</div>
