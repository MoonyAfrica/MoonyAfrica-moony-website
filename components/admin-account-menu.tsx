...
import { ChevronDown, KeyRound, LogOut, ShieldCheck, UserCog, UserRound } from "lucide-react";

type Session={name:string;email:string;role:string;legacy?:boolean;mfa?:boolean};
...
export function AdminAccountMenu(){
 ...
 return <div className="relative hidden border-l border-[#5b2f22]/10 pl-4 sm:block">
  ...
  {open?<div className="absolute right-0 top-12 z-50 w-72 rounded-2xl border border-[#5b2f22]/10 bg-white p-3 shadow-[0_20px_60px_rgba(52,26,18,.14)]">
   ...
   <div className="mt-2 space-y-1 border-t border-[#5b2f22]/8 pt-2">
    <Link onClick={()=>setOpen(false)} href="/admin/mon-compte" className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs text-[#5b2f22]/75 hover:bg-[#fbf4ee]"><UserRound size={14}/>Mon compte & sécurité</Link>
    {session?.role==="founder"?<Link onClick={()=>setOpen(false)} href="/admin/equipe" className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs text-[#5b2f22]/75 hover:bg-[#fbf4ee]"><UserCog size={14}/>Équipe & rôles</Link>:null}
    <button onClick={()=>void logout()} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs text-red-700 hover:bg-red-50"><LogOut size={14}/>Se déconnecter</button>
   </div>
  </div>:null}
 </div>
}
