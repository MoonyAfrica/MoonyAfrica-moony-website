import { effectiveProposalStatus, loadProposalPortalDocument, proposalIsExpired } from "@/lib/crm-proposal-portal";
...
export async function notifyCrmProposalEvent(supabase:any,input:{proposalId:string;opportunityId:string;title:string;subtitle:string;severity?:"info"|"warning"|"urgent"}){
  ...
  for(const role of CRM_NOTIFICATION_ROLES){
    const duplicate=await supabase.from("control_center_generated_notifications").select("id").eq("target_role",role).eq("source_type","crm_proposal").eq("source_id",`${input.proposalId}:${input.title}`).limit(1);
    if(!duplicate.error&&duplicate.data?.length)continue;
    const inserted=await supabase.from("control_center_generated_notifications").insert({target_role:role,title:input.title,subtitle:input.subtitle,href:`/admin/propositions?proposal=${input.proposalId}`,severity:input.severity||"info",source_type:"crm_proposal",source_id:`${input.proposalId}:${input.title}`});
...
export async function runProposalFollowups(supabase:any):Promise<ProposalFollowupSummary>{
  ...
  let expired=0,viewedFollowups=0,expiryReminders=0,notifications=0;const errors:string[]=[];const now=new Date();
  for(const row of result.data??[]){
    ...
    if(!settings.auto_reminders||Number(row.reminder_count||0)>=settings.max_reminders||!row.public_link_enabled||!row.public_token_hash)continue;
    const document=await loadProposalPortalDocument(supabase,String(row.id));if(!document)continue;
    const origin=process.env.NEXT_PUBLIC_SITE_URL||"";
    if(!origin)continue;
    const tokenUnavailable=true;
    // Public reminder e-mails can only reuse a client URL if the raw token is available at send time.
    // Because MOONY intentionally stores only a hash, automatic reminders request a fresh secure link through the client-link rotation endpoint instead of reconstructing tokens here.
    if(tokenUnavailable)continue;
  }
