...
import { getFxBundle } from "@/lib/crm-fx";
...
export type ExecutiveMetrics={
 ...
 fx:{available:boolean;baseCurrency:string;rates:Record<string,number>;rateDates:Record<string,string>;providers:Record<string,string[]>;source:string;fetchedAt:string|null};
 ...
 dataQuality:{revenue:boolean;commercial:boolean;customerSuccess:boolean;renewalForecast:boolean;voiceOfCustomer:boolean;escalations:boolean;fxRates:boolean;notes:string[]};
};
...
export async function buildExecutiveMetrics(supabase:any,periodMonth:string):Promise<ExecutiveMetrics>{
 ...
 const fxBundle=await getFxBundle(supabase);const fx={available:fxBundle.available,baseCurrency:fxBundle.baseCurrency,rates:fxBundle.rates,rateDates:fxBundle.rateDates,providers:fxBundle.providers,source:fxBundle.source,fetchedAt:fxBundle.fetchedAt};if(!fx.available)notes.push(`Taux FX indisponibles : ${fxBundle.error||"source de change indisponible"}. Les montants originaux restent valides et séparés par devise.`);
